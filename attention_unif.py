...
class AttentionLayerWithBatchNormalization(Layer):
    def __init__(self, **kwargs):
        super(AttentionLayerWithBatchNormalization, self).__init__(**kwargs)
    ...
    def build(self, inputs_shape):
        ...
        # Expected input shape consists of a triplet: (batch, input_length, input_dim)
        if len(input_shape) != 3:
            raise ValueError("Input shape for AttentionLayerWithBatchNormalization should be of 3 dimension.")
        ...
        self.attention_param = self.add_weight(
            ...
            trainable=True,
            dtype=tf.float32)
        super(AttentionLayerWithBatchNormalization, self).build(input_shape)
